...
import { Villa } from '../types/index.js';
...
export default class VillaModel {
    ...
    static async createVilla(villaData: Villa): Promise<Villa> {
        ...
        try {
            ...
            const { rows } = await db.query(
                'INSERT INTO villas (name, description, address, capacity, bathrooms, rooms, price, images) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id;',
                [name, safeDescription, address, capacity, bathrooms, rooms, price, imagesJson]
            );

            const { id, ...dataWithoutId } = villaData;
    ...
    static async updateVilla(id: number, villaData: Partial<Villa>): Promise<Villa> {
        const { name, description, address, capacity, bathrooms, rooms, price, images } = villaData;
        const updateFields = [];
        const updatedValues = [];
        let paramCount = 1;
